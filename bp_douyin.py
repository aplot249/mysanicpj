    ...
    async with aiohttp.ClientSession() as session:
        ...
        with open(absolute_path.resolve(), 'wb') as fd:
            while True:
                ...
                os.fsync(fd.fileno())
        # #上传
        await get_event_loop().run_in_executor(None, client.put_object_from_local_file, 'video', absolute_path,
                                               absolute_path.name)
...
async def jiekou2(share_link, res_dict):
    async with aiohttp.ClientSession() as session:
        ...
        response = await session.get(
            f"https://3g.gljlw.com/diy/ttxs_dy2.php?url={share_link}&r=011715320318362199&s=bbf3165ccbd383d6587fefdbfffb8844",
            headers=headers)
        res = await response.text()
